...
const screens={title:<HomeScreen onStart={back}/>,home:<MenuScreen onPlay={()=>set('matching')} onTutorial={()=>set('tutorial')} onTsume={()=>set('tsume')} onSkins={()=>set('skins')} onBattlePass={()=>set('pass')} onMissions={()=>set('missions')} onShop={()=>set('shopscreen')} onLetters={()=>set('letters')}/>,matching:<MatchingScreen onOnline={()=>set('online')} onFriend={()=>set('room')} onCpu={()=>set('rules')} onBack={back} onTutorial={()=>set('tutorial')} onRanking={()=>set('ranking')}/>,rules:<RulesSelectScreen onStart={back} onBack={back} backLabel="戻る"/>,online:<RandomMatchScreen onBack={back} onRoomReady={back} boardSize={9}/>,room:<RoomScreen onBack={back} onRoomReady={back} boardSize={9}/>,tutorial:<TutorialSelect onBack={back} onStart={back}/>,missions:<MissionsScreen onBack={back}/>,tsume:<TsumeScreen onBack={back}/>,pass:<BattlePassScreen onBack={back} onSkins={()=>set('skins')}/>,skins:<SkinsScreen onBack={back} onBattlePass={()=>set('pass')}/>,ranking:<RankingScreen onBack={back}/>,letters:<LettersScreen onBack={back}/>,shop:<GemShop gems={1250} gemsPaid={1000} gemsFree={250} onClose={back} onMessage={()=>{}}/>,shopscreen:<ShopScreen onBack={back} onGacha={()=>set('skins')} onFoil={()=>set('skins')} onBattlePass={()=>set('pass')}/>,settings:<SettingsModal onClose={back}/>,icons:<IconPickModal onClose={back}/>,titles:<TitlePickModal onClose={back}/>,guide:<RulesPanel onClose={back}/>};content=screens[page]||screens.home;return <GameShell onHome={back} showRules={rules} setShowRules={setRules}>{content}</GameShell>};createRoot(document.getElementById('root')).render(<App/>);
`,
    loader: "jsx",
  },
});
