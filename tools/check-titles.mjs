...
is("既定の称号は最初から使える", !!TITLES.find((t) => t.id === DEFAULT_TITLE && t.free), true);
is(
  "手に入れ方が全部書いてある",
...
is("1局遊ぶと初陣が新しく手に入る", r.earned.map((t) => t.name), ["初陣"]);
...
r = recordGame(true);
is("10勝で十勝の勇", r.earned.map((t) => t.name), ["十勝の勇"]);
...
is("同じ称号を2度配っても1つ", grantTitle("rank-o").titles, ["rank-o"]);
...
console.log("壊れた保存");
store["tottery.account.v1"] = JSON.stringify({ ...loadProfile(), title: "brand-new" });
is("知らない称号が保存されていたら既定に戻す", titleOf(loadProfile()).name, "見習い");
