...
NOLINES = len(sys.argv) > 8 and sys.argv[8] == 'nolines'
NOTES = None if len(sys.argv) <= 9 else json.loads(sys.argv[9])
def board(x0, y0, title, grid, cell=54):
    ...
    for dr,dc in ([] if NOLINES else [(-1,0),(0,1),(-1,1)]):
...
notes=NOTES or [("王","#d9a441",["いちばん奥の隅に置く。縦・横・斜めの線（点線）はすべて味方で塞がり、","桂馬で跳び込める2升にも味方が居るので、どのランクの駒も王に届かない"]),
