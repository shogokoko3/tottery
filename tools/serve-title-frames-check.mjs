...
import {FRAME_GRADES,titleDesign} from './src/ui/title-design.js';
...
seed(true);
function App(){
 const [page,setPage]=useState('difficulty'),[picker,setPicker]=useState(false),[profile,setProfile]=useState(loadProfile),[foe,setFoe]=useState('foil-demon-k');
 const samples=['novice','ten','win30','rank-sho','rank-o','season:2026-09:first'];
...
if (process.argv.includes("--export")) {
  const destination = path.resolve("reports/title-frames/index.html");
  fs.mkdirSync(path.dirname(destination), { recursive: true });
  fs.writeFileSync(destination, html);
  console.log(`Standalone title catalogue: ${destination}`);
  process.exit(0);
}
http
