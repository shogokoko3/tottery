...
for (const legend of [false, true]) {
  ...
  for (const elapsed of [plan.reveal, plan.settle, plan.complete])
    assert.equal(foilUnveilingFrame(elapsed, { legend }).identityVisible, true);
  assert.equal(
    foilUnveilingFrame(plan.complete - 1, { legend }).complete,
    false,
  );
  assert.equal(foilUnveilingFrame(0, { legend, reduce: true }).complete, true);
  let time = 0,
    finish = 0,
    frame,
    next = 0;
