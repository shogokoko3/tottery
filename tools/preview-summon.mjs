...
const flowVersion = createHash("sha256")
  .update(flow.outputFiles[0].text)
  .digest("hex")
  .slice(0, 12);
fs.writeFileSync(`${out}/flow.js`, flow.outputFiles[0].text);
fs.writeFileSync(
  `${out}/flow.html`,
  '<!doctype html><html lang="ja"><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1,viewport-fit=cover"><title>召喚 → 結果カード · プレビュー</title><div id="root"></div><script src="flow.js"></script></html>'.replace(
    'src="flow.js"',
    `src="flow.js?v=${flowVersion}"`,
  ),
);
