...
const RANK_OF = Object.fromEntries(
  TYPES.map((t) => [
    t,
    Object.keys(AREA_BY_RANK).find((r) => AREA_BY_RANK[r] === t),
  ]),
);
...
function play(seed, kings, hasArea) {
  ...
  const all = Object.fromEntries(
    Object.keys(AREA_BY_RANK).map((r) => [r, "skin:foil"]),
  );
  let s = reducer(
    { phase: "intro" },
    {
      ...
      // いまの決まり(版)で測る。旧版のままだと海・宮殿などが旧処理になる
    ...
    if (s.phase === "setup") {
      for (const idx of [0, 1]) {
        ...
        if (!wanted) {
          ...
          const out = Object.keys(placement).find(
            (id) => me.hand.find((x) => x.id === id)?.rank !== "K",
          );
          if (card && out) {
            placement[card.id] = placement[out];
            delete placement[out];
            wanted = card;
          }
        }
        step({
  ...
  return {
    ...
    used: (s.areas || []).map((a) => !!(a && a.uses)),
  };
}

console.log(
  `盤面エリアの強さ(CPU同士、9×9、各 ${PAIRS * 2} 局。相手はエリア無し)`,
);
