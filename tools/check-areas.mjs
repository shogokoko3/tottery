...
function startGame({
  ...
  if (deckOrder) deck = deckOrder(deck);
    ...
    const pick = (rank, used) =>
      deck.find((c) => c.rank === rank && !used.has(c.id));
    const used = new Set();
    const a = pick(kings[0], used);
    used.add(a.id);
    const b = pick(kings[1], used);
    used.add(b.id);
  ...
  while (s.phase !== "play" && guard++ < 60) {
    ...
    if (s.phase === "mulligan") {
      step({ type: "CONFIRM_MULLIGAN", discardIds: [] });
      continue;
    }
    if (s.phase === "setup") {
      for (const idx of [0, 1]) {
        ...
        const handOrder = me.hand
          .filter((c) => c.rank !== "K" || kings[idx] === "K")
          .sort(
            (a, b) =>
              Number(b.rank === kings[idx]) - Number(a.rank === kings[idx]),
          )
          .map((c) => c.id);
...
const mine = (s, p) =>
  Object.values(s.pieces).filter((x) => x.alive && x.owner === p);
const anyMove = (s, p, filter = () => true) => {
  for (const piece of mine(s, p)) {
    ...
    const moves = getLegalMoves(
      ...
      s.board,
      s.boardSize,
      s.players[p].armyRankCounts,
      kingRankOf(s, p),
    );
    // 取りは避ける(盤を単純に保つ)
    const quiet = moves.find(
      (m) => !s.board[m.row][m.col] && !m.captures?.length,
    );
    if (quiet)
      return {
        type: "MOVE_PIECE",
...
console.log("旗が無ければ何も起きない");
{
  ...
  is(
    "USE_AREA は無視",
    reducer(s, { type: "USE_AREA", hit: true }) === s,
    true,
  );
  ...
  is(
    "王のランクにスキンが無い側は立たない",
    [u.areas[0] && u.areas[0].type, u.areas[1]],
    ["earth", null],
  );
  const v = startGame({
    kings: ["2", "3"],
    loadouts: [{ 2: "zombie-male" }, { 3: "zombie-female:foil" }],
  });
  is(
    "通常のスキンでは立たず、フォイルでだけ立つ",
    [v.areas[0], v.areas[1] && v.areas[1].type],
    [null, "earth"],
  );
...
console.log("土: 足跡を読む");
{
  ...
  // 相手がまだ動いていなければ読めない
  if (s.currentTurn === 0)
    is("足跡が無いと使えない", canUseArea(s, 0).ok, false);
  else is("相手の番には使えない", canUseArea(s, 0).ok, false);
  // 土の側の番になるまで進める(相手が動いた直後)
  while (s.currentTurn !== 0 || !s.lastMove || s.lastMove.owner !== 1)
    s = playQuiet(s);
  is(
    "相手の直前の駒は公開されていない(前提)",
    !!s.board[s.lastMove.to.row][s.lastMove.to.col].revealed,
    false,
  );
  ...
  is(
    "外れ: 正体は分からない",
    isKnownTo(miss, 0, miss.pieces[target.id]),
    false,
  );
  ...
  is(
    "当たり: 自分だけが知る",
    [
      isKnownTo(hit, 0, hit.pieces[target.id]),
      isKnownTo(hit, 1, hit.pieces[target.id]) === true &&
        hit.pieces[target.id].owner === 1,
    ],
    [true, true],
  );
  ...
  is(
    "記録に残り、当たり外れは相手にも分かる",
    [
      hit.log.at(-1).includes("見抜いた"),
      miss.log.at(-1).includes("読み違えた"),
    ],
    [true, true],
  );
  is(
    "2回目は使えない",
    reducer(hit, { type: "USE_AREA", hit: true }) === hit,
    true,
  );
  is(
    "通信で届く手に hit が無ければ捨てる",
    reducer(s, { type: "USE_AREA", player: 0 }) === s,
    true,
  );
  is(
    "enrichAction が hit を焼き込む",
    typeof enrichAction({ type: "USE_AREA" }, s).hit,
    "boolean",
  );
  is(
    "版11: 足跡を読めば必ず見抜く(earthOdds 1)",
    AREA_TUNING.earthOdds >= 1 && enrichAction({ type: "USE_AREA" }, s).hit,
    true,
  );
}
...
console.log("海: 中央へ引き寄せる");
{
  ...
  const before = Object.fromEntries(
    Object.values(s.pieces).map((p) => [p.id, cheb(p)]),
  );
  ...
  is(
    "駒は減らない",
    Object.values(t.pieces).filter((p) => p.alive).length,
    alive,
  );
  let overlap = 0,
    farther = 0,
  ...
  const near = Object.values(t.pieces).filter(
    (p) => p.alive && cheb(p) <= 2,
  ).length;
...
console.log("海(版11): 相手の駒だけ流される");
{
  ...
  const cheb = (p) => Math.max(Math.abs(p.row - c), Math.abs(p.col - c));
  const before = Object.fromEntries(
    Object.values(s.pieces).map((p) => [
      p.id,
      { row: p.row, col: p.col, d: cheb(p) },
    ]),
  );
  const t = reducer(s, { type: "USE_AREA" });
  const mine = Object.values(t.pieces).filter((p) => p.alive && p.owner === 0);
  const foes = Object.values(t.pieces).filter((p) => p.alive && p.owner === 1);
  is(
    "自分の駒は1体も動かない",
    mine.every((p) => p.row === before[p.id].row && p.col === before[p.id].col),
    true,
  );
  is(
    "相手の駒は中央方向へ最大1マス",
    foes.every(
      (p) =>
        Math.max(
          Math.abs(p.row - before[p.id].row),
          Math.abs(p.col - before[p.id].col),
        ) <= 1 && cheb(p) <= before[p.id].d,
    ),
    true,
  );
  is(
    "相手の駒のどれかは動く",
    foes.some((p) => p.row !== before[p.id].row || p.col !== before[p.id].col),
    true,
  );
  is("記録は相手の駒", t.log.at(-1).includes("相手の駒を中央へ"), true);
  const v9 = reducer({ ...s, ruleVersion: 10 }, { type: "USE_AREA" });
  is(
    "版10の対局は自分の駒も流される",
    Object.values(v9.pieces).some(
      (p) =>
        p.alive &&
        p.owner === 0 &&
        (p.row !== before[p.id].row || p.col !== before[p.id].col),
    ),
    true,
  );
}
...
console.log("森: 1体見抜く");
{
  ...
  is(
    "enrichAction が並びを焼き込む",
    Array.isArray(act.picks) && act.picks.length,
    8,
  );
  if (!act.picks) {
    console.log("  (森が立っていないので以降を飛ばす)", s.areas);
    process.exit(1);
  }
  ...
  is(
    "相手の駒で王ではない",
    known.every((id) => t.pieces[id].owner === 1 && !t.pieces[id].isKing),
    true,
  );
  ...
  is(
    "王や知らない id は無視され、固定の並びで補う",
    Object.keys(bad.known[0]).length === 1 && !bad.known[0][kingId],
    true,
  );
}
...
console.log("氷: 乱数で1体、3手番動けない");
{
  ...
  is(
    "王や知らない id は無視され、固定の並びで補う",
    mine(forced, 1).filter((p) => isFrozen(forced, p)).length === 1 &&
      !isFrozen(forced, forced.pieces[foeKing]),
    true,
  );
  // 自分が1手指して相手の番に
  let u = playQuiet(t);
  ...
  if (moves.length) {
    ...
    is(
      "凍った駒は選べない",
      reducer(u, { type: "SELECT_PIECE", id: frozenPiece.id }) === u,
      true,
    );
  }
  // 相手の手番を3回数える
  ...
  while (foeTurns < FREEZE_TURNS && guard++ < 20) {
    if (u.currentTurn === 1) {
      is(
        `相手の${foeTurns + 1}手番目: まだ凍っている`,
        isFrozen(u, u.pieces[frozenPiece.id]),
        true,
      );
      foeTurns++;
    }
...
console.log("氷: A の入れ替えで解ける。凍った A は使えない");
{
  ...
  const other = mine(s, 1).find(
    (p) => !p.isKing && p.id !== ace.id && p.id !== target.id,
  );
  let t = reducer(s, { type: "USE_AREA", picks: [target.id] });
  is("狙いの駒が凍った", isFrozen(t, t.pieces[target.id]), true);
  t = playQuiet(t);
  is("相手の番", t.currentTurn, 1);
  const swapped = reducer(t, {
    type: "CONFIRM_SHUFFLE",
    aId: ace.id,
    pickIds: [target.id, other.id],
    order: [1, 2, 0],
  });
  is("凍った駒も入れ替えに使える", swapped !== t, true);
  is(
    "入れ替えで氷が解ける",
    isFrozen(swapped, swapped.pieces[target.id]),
    false,
  );
  is(
    "履歴に残る",
    swapped.pieces[target.id].history.includes("入れ替えで氷が解けた"),
    true,
  );
  // 凍った A 自身は入れ替えを使えない
  const frozenAce = {
    ...
    pieces: {
      ...t.pieces,
      [ace.id]: { ...t.pieces[ace.id], frozenUntil: t.turnNo + 6 },
    },
  };
  is(
    "凍った A は入れ替えを使えない",
...
console.log("氷: 凍らされて何も指せなければ負け");
{
  ...
  const u = {
    ...t,
    pieces: { ...t.pieces },
    board: t.board.map((r) => r.map(() => null)),
  };
  const foe = mine(t, 1);
  ...
  others.forEach((p, i) =>
    i < 4 ? place(p, ...around[i]) : (u.pieces[p.id] = { ...p, alive: false }),
  );
  for (const p of others.slice(4))
    u.players = u.players.map((pl, i) =>
      i === 1 ? { ...pl, capturedOwn: [...pl.capturedOwn, p.id] } : pl,
    );
  // 自分の駒は端に並べ直す
  ...
  is(
    "手番が回った時点で負け",
    [v.phase, v.winner, v.endReason],
    ["gameover", 0, "frozen"],
  );
  is(
    "記録に残る",
    v.log.some((l) => l.includes("凍りついて動けない")),
    true,
  );
}
...
console.log("空: 本物の10に変身(公開)、軍の10は全て2回動く");
{
  ...
  s = {
    ...s,
    areas: [{ type: "sky", used: false, rank: "J", skin: "x" }, null],
  };
  // ランダム布陣で最初の駒の跳躍先が全て味方に埋まる場合がある。
  // 2回移動の検査には、空きマスへ跳べる変身対象を選ぶ。
  const target = mine(s, 0).find(
    (p) =>
      !p.isKing &&
      p.rank !== "10" &&
      p.rank !== "A" &&
      getLegalMoves(
        ...
        kingRankOf(s, 0),
      ).some((m) => !s.board[m.row][m.col]),
  );
  const before = s.players[0].armyRankCounts;
  is(
    "王は候補にならない",
    reducer(s, { type: "USE_AREA", pieceId: s.players[0].kingId }) === s,
    true,
  );
  ...
  is(
    "採用枚数の表も追いかける",
    [
      t.players[0].armyRankCounts[target.rank] || 0,
      t.players[0].armyRankCounts["10"] || 0,
    ],
    [(before[target.rank] || 0) - 1, (before["10"] || 0) + 1],
  );
  ...
  is(
    "桂馬跳びになる",
    moves.every(
      (m) => Math.abs(m.row - piece.row) + Math.abs(m.col - piece.col) === 3,
    ),
    true,
  );
  is("手番は消費しない", t.currentTurn, 0);
  ...
  is(
    ...
    reducer(
      u,
      anyMove(u, 0, (p) => p.id !== piece.id),
    ) === u,
    true,
  );
  const again = getLegalMoves(
    u.pieces[piece.id],
    u.board,
    9,
    u.players[0].armyRankCounts,
    kingRankOf(u, 0),
  ).find((m) => !u.board[m.row][m.col]);
  u = reducer(u, {
    type: "MOVE_PIECE",
    pieceId: piece.id,
    row: again.row,
  ...
  is(
    "記録に2回目が残る",
    u.log.some((l) => l.includes("2回目に移動")),
    true,
  );
  // 元からいる10も2回動ける
  const other10 = mine(t, 0).find(
    (p) => p.rank === "10" && p.id !== piece.id && !p.isKing,
  );
  if (other10) {
...
console.log("宮殿: 昇格は手番を使う");
{
  ...
  is(
    "promotedRank は K まで",
    ["2", "9", "10", "J", "Q", "K", "A"].map(promotedRank),
    ["3", "10", "J", "Q", "K", null, null],
  );
  ...
  is(
    "公開され、専用のしるしが付く",
    [t.pieces[nine.id].revealed, t.pieces[nine.id].mark],
    [true, "palace"],
  );
  is(
    "採用枚数の表も追いかける",
    [
      t.players[0].armyRankCounts[nine.rank] || 0,
      t.players[0].armyRankCounts[up] || 0,
    ],
    [(before[nine.rank] || 0) - 1, (before[up] || 0) + 1],
  );
  ...
  const ten = mine(s, 0).find(
    (p) => p.rank === "10" || p.rank === "J" || p.rank === "Q",
  );
  if (ten) {
    const w = reducer(s, { type: "USE_AREA", pieceId: ten.id });
    is(
      "10 以上も1段上がる(採用枚数の制限は見ない)",
      w.pieces[ten.id].rank,
      promotedRank(ten.rank),
    );
  }
  ...
  if (kk)
    is(
      "K はもう上がらない",
      reducer(s, { type: "USE_AREA", pieceId: kk.id }) === s,
      true,
    );
  is(
    "王は昇格できない",
...
console.log("使えない場面");
{
  ...
  is(
    "通信で届いた相手の名乗りは捨てる",
    reducer(s, { type: "USE_AREA", player: 1 - p }) === s,
    true,
  );
  is(
    "対局中以外は使えない",
    canUseArea({ ...s, phase: "gameover" }, p).ok,
    false,
  );
}
...
console.log("再生: 同じ手の列で同じ盤になる(オンラインの前提)");
{
  ...
  let rnd = seed;
  const random = () =>
    (rnd = (rnd * 1103515245 + 12345) % 2147483648) / 2147483648;
...
console.log("CPU 同士で回しても盤が壊れない");
{
  ...
  for (let g = 0; g < GAMES; g++) {
    ...
    let s = startGame({
      kings: [ranks[g % ranks.length], ranks[(g * 5 + 1) % ranks.length]],
    });
    let guard = 0;
    while (s.phase !== "gameover" && guard++ < 400) {
      s = dismiss(s);
      if (s.clocks[s.currentTurn] <= 0) {
      ...
      if (act.type === "MOVE_PIECE" && isFrozen(s, s.pieces[act.pieceId]))
        frozenMoves++;
      const next = reducer(s, enrichAction({ ...act, elapsedMs: 1000 }, s));
      if (
        act.type === "USE_AREA" &&
        next.lastArea &&
        next.lastArea.type === "palace"
      )
        promoted++;
      s = next;
      // 不変条件
      ...
      for (const p of Object.values(s.pieces)) {
        if (!p.alive) continue;
        if (
          !inBounds(p.row, p.col, 9) ||
          !s.board[p.row][p.col] ||
          s.board[p.row][p.col].id !== p.id
        )
          problems++;
        const key = `${p.row},${p.col}`;
