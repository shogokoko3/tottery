...
is("はじめに挑戦できるのは縦横の4マス", ids(openCells(s)), ["1-2", "2-1", "2-3", "3-2"]);
is("斜めのマスはまだ開かない", openCells(s).some((c) => c.id === "1-1"), false);
...
is("クリアすると隣が開く", openCells(s).some((c) => c.id === "0-2"), true);
...
is("1枚残っていれば受け取れない", [allFlipped(full), canClaim(full)], [false, false]);
full = flip(full, "0-0");
...
console.log("保存の読み直し");
is("知らない id は落とす", normalize({ cleared: ["9-9"], flipped: ["9-9"] }).cleared, [centerId]);
is("クリアしていないマスのめくりは落とす", normalize({ flipped: ["1-2"] }).flipped, []);
is("壊れた進みは落とす", normalize({ progress: { "1-2": -3, "9-9": 5 } }).progress, {});
is("読み直しても同じ", normalize(normalize(full)).flipped.length, CELLS.length);

console.log("対局から拾う");
const piece = (id, owner, rank, alive, isKing) => ({ id, owner, rank, alive, isKing });
const bef = {
  currentTurn: 0,
  pieces: {
    a: piece("a", 1, "5", true),
...
is(
  ...
  [{ rank: "5", wasKing: false }, { rank: "K", wasKing: true }],
);
is("相手の手番では拾わない", capturedIn({ ...bef, currentTurn: 1 }, aft, 0), null);
is("何も減っていなければ null", capturedIn(bef, { pieces: bef.pieces }, 0), null);
is("盤が無くても落ちない", [capturedIn(null, aft, 0), capturedIn(bef, null, 0)], [null, null]);
