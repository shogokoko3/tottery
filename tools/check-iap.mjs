...
is("おまけ率は買うほど上がる(非減少)", (() => { const r = PRODUCTS.map((p) => p.free / p.paid); return r.every((v, i) => i === 0 || v >= r[i - 1] - 1e-9); })(), true);
  is("いちばん大きいパックのおまけは30%", (() => { const p = PRODUCTS[PRODUCTS.length - 1]; return p.free === Math.round(p.paid * 0.3); })(), true);
  is("おまけはどれも30%を超えない", PRODUCTS.every((p) => p.free <= p.paid * 0.3 + 1e-9), true);
is("おまけ率は買うほど上がる(非減少)", (() => { const r = PRODUCTS.map((p) => p.free / p.paid); return r.every((v, i) => i === 0 || v >= r[i - 1] - 1e-9); })(), true);
