...
export function formationFixture(form = "wings") {
  ...
  if (form === "fortress") {
    ...
    [
      ...
      [6, 1],
      [6, 2],
    ].forEach(([row, col], i) => specs.push([ranks[i], 0, row, col, i === 0]));
  } else {
    const def =
      FORMATIONS.find(
        (d) =>
  ...
  specs.forEach(([rank, owner, row, col, isKing], i) => {
    ...
    s.players[owner].armyRankCounts[rank] =
      (s.players[owner].armyRankCounts[rank] || 0) + 1;
  });
  const area =
    form === "fortress"
      ? "ice"
    ...
    areas: [
      {
        ...
        uses: 0,
        used: false,
        skin: form === "hell" ? "demon-k:foil" : "angel-k:foil",
      },
      null,
    ],
    known: [{}, {}],
