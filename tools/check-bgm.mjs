...
is(
  trackForScene({ screen: "game", phase: "play" }),
  "battle",
  "対局は対局の曲",
);
is(
...
is(
  trackForScene({ screen: "game", phase: "gameover", result: "draw" }),
  "waiting",
  "引き分けは勝敗ジングルを鳴らさない",
);
...
is(
  isEndgame({ pieces }, [300000, 300000]),
  false,
  "駒も時間も残っていれば終盤ではない",
);
is(
  isEndgame({ pieces }, [300000, ENDGAME_CLOCK_MS]),
  true,
  "持ち時間が尽きかけたら終盤",
);
const few = {};
...
const kingHurt = {};
for (let i = 0; i < 10; i++)
  kingHurt[i] = { alive: true, isKing: i === 0, hp: i === 0 ? 1 : 9 };
is(
  isEndgame({ pieces: kingHurt }, [300000, 300000]),
  false,
  "王の具合では曲を変えない",
);
