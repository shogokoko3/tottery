 ...
 * 盤面エリア。
 *
 * 王にした札のランクに**フォイルの**スキン(ガチャ・錬成・通算入手報酬の加工版。id の末尾が
 ...
 *   J〜K  宮殿 手番を使わず、自分の駒1体(王・A・K以外)を1段昇格させる(Kまで)。
 *              1試合に1回だけ2段階も選べる。昇格した駒は公開される
 *
 * 空と宮殿で現れた札は公開(revealed)になり、専用のしるし(piece.mark)が付く。
 * 土・森・氷は発動可能な自分の手番の初めに自動発動する。
 * 海は任意発動。空・宮殿も任意発動で対象を選ぶ。全エリアとも手番を消費しない(旧版の宮殿を除く)。
 * 発動は相手にも見える。見抜いた正体だけは自分にしか見えない(state.known)。
 *
 * 土の成否(現行版は必ず成功)と森・氷の抽選結果は src/game/actions.js の enrichAction で
 * 手に焼き込む。オンラインでは両者が同じ手を再生するので、ここは決定的に動く。
 *
 * START_SETUP に `areas: true` と両者の装備
...
export const AREA_INFO = Object.freeze({
  earth: {
    name: "土のエリア",
    text: "毎回の自分の手番開始時、直前に動いた相手の駒の足跡を読み、まだ知らない正体を必ず見抜く。王も対象。見抜いた正体は自分だけが知る。対象がいない場合は発動しない。",
    usesTurn: false,
    needsPiece: false,
  },
  sea: {
    name: "海のエリア",
    text: "毎回の自分の手番開始時、任意で1回発動。相手の全ての駒を中央方向へ最大1マスずつ引き寄せる。自分の駒は流されず、行き先が埋まった駒はその場に留まる。この効果では駒を取らない。発動後も駒を動かせる。",
    usesTurn: false,
    needsPiece: false,
  },
  forest: {
    name: "森のエリア",
    text: "毎回の自分の手番開始時、相手の王以外で、まだ正体を知らない駒をランダムで2体見抜く。候補が1体なら1体、いなければ発動しない。対象と正体は自分だけが知る。",
    usesTurn: false,
    needsPiece: false,
  },
  ice: {
  ...
  sky: {
    name: "空のエリア",
    text: "毎回の自分の手番開始時、任意で自分の駒1体(王・10以外)を10に変身させて公開する。変身後も駒を動かせる。以後、自軍の10はどれでも、その手番に選んだ同じ1体を2回動かせる。",
    usesTurn: false,
    needsPiece: true,
  },
  palace: {
    name: "宮殿",
    text: "毎回の自分の手番開始時、任意で自分の駒1体(王・A・K以外)を1段階昇格させて公開する。各プレイヤーは1試合に1回だけ、代わりに2段階昇格も選べる(Kが上限)。昇格後も駒を動かせる。",
    usesTurn: false,
    needsPiece: true,
  },
});
...
/**
 * エリアを立てられる装備か。入手経路を問わずフォイル加工のスキンだけ。
