...
export const AREA_GUIDE_ROWS = [
  {
    ...
    ranks: "2・3",
    when: "自分の手番の初めに自動",
    effect: `直前に動いた相手の駒の足跡を読み、${AREA_TUNING.earthOdds >= 1 ? "必ず" : `${Math.round(AREA_TUNING.earthOdds * 100)}%で`}正体を見抜きます。見抜いた駒は自分にだけ表向きになります。`,
  ...
  },
  {
    ...
    effect: `${AREA_TUNING.seaPullsOwn ? "盤上の全ての駒" : "相手の駒"}を、中央に向かって最大1マスずつ引き寄せます。${AREA_TUNING.seaPullsOwn ? "" : "自分の駒は流されません。"}行き先が埋まっている駒はその場に残ります。取りは起きません。発動後も駒を動かせます。使わずに駒を動かしてもよく、次の手番にはまた選べます。`,
