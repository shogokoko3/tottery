  ...
  return (
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal-panel" onClick={(n) => n.stopPropagation()}>
        ...
        <p className="hint rule-foot">
          <b>終局できない局面の判定：</b>駒の移動範囲から両者の王を討てないと確定した場合、または動く側に合法な行動がない場合は、対局開始時に採用した札の数字の合計が低い側の勝ちです。同点は引き分け。A=1、J=11、Q=12、K=13として数え、倒れた駒も含みます。途中で投入した予備札は合計に含みません。同じ局面の繰り返しだけでは判定しません。
        </p>
      </div>
    </div>
  );
