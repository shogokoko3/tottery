...
export const AREA_GUIDE_ROWS = [
  ...
  },
  {
    ...
    when: "自分の手番の初めに「発動」を押して駒を選ぶ",
    effect:
      "自分の駒1体(王以外)を1段昇格させます(2→3 … 9→10→J→Q→K)。昇格した駒は表向きになり「宮」の印が付きます。昇格したあと、その手番で普通に駒を動かせます。",
  ...
  return (
    <div className="area-guide">
      <p className="hint area-guide-lead">
        <b>9×9 の対局だけ</b>のしくみです。ガチャで引ける<b>フォイル加工のスキン</b>を
        王に選んだ札のランクに装備していると、そのランク帯の「エリア」が自陣に立ちます
        （通常のスキンでは立ちません）。エリアは自分にも相手にも見え、
        <b>毎回の自分の手番に1回</b>効果が出ます。5×5 とチュートリアルにはありません。
      </p>
      <div className="area-guide-list">
        {AREA_GUIDE_ROWS.map((row) => (
          <div className={`area-guide-row area-guide-${row.type}`} key={row.type}>
      ...
      <p className="hint rule-foot">
        <b>印の読み方：</b>「見抜」は自分だけが正体を知っている相手の駒。❄ は凍っている駒。
        「空」「宮」は変身・昇格で正体が変わった駒で、こちらは相手にも見えています。
      </p>
      <p className="hint rule-foot">
        <b>手に入れ方：</b>ガチャで、その帯のどちらかのランクのフォイルを引く
        （たとえば土なら 2 か 3 のフォイル）。引いたフォイルをそのランクに装備し、
        その札を王にすると発動します。効果の中身は、スキン画面でそのフォイルを
        開くと確かめられます。
      </p>
      <p className="hint rule-foot">
        <b>オンラインでは</b>、二人とも新しい版のアプリのときだけエリアが立ちます。
