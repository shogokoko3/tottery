 ...
 * 抽選の中身は R 4枚で65%、SR 4枚で32%、SSR 6枚で3%。1枚あたりだと
 *   R 16.25% / SR 8% / SSR 0.5%
 * R を 10 とすると SR は 16.25÷8 ≒ 2倍、SSR は 16.25÷0.5 = 32.5倍。
 * 端数を丸めて 10 / 20 / 320 にした。
 *
 * ■ 生成は分解の4倍
 * 同じ格なら「ダブり4枚で、好きな1枚」。R 40 / SR 80 / SSR 1280。
 * 4倍にしたのは、1倍だと交換所になって引く意味が消え、
 * 10倍だと崩しても届かず素材が死ぬため。
 *
 * ■ その結果どうなるか(1回引くごとに入るエーテル)
 *   R から 0.65×10 = 6.5 / SR から 0.32×20 = 6.4 / SSR から 0.03×320 = 9.6
 * なので
 *   R だけを崩して SSR 1枚(1280)  → 197回
 *   SR だけを崩して SSR 1枚       → 200回
 *   狙った SSR を運で当てる        → 200回   ← ほぼ同じ手間
 *   引いたものを全部崩す           → 57回    ← 運任せの3.5倍の速さ
...
/** 分解でもらえる量 */
export const DUST = Object.freeze({ R: 10, SR: 20, SSR: 320 });
