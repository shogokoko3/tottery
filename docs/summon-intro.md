...
導入の右下に「スキップ」を表示する。読み込み中・階段・門・飛翔中のどの時点でも押せ、門の描画と効果音を終了して、カードをめくる画面へ進む。1回・10連とも全カードは伏せた状態を保ち、抽選・消費・所持品は変更しない。カード開示画面に演出スキップボタンは設けず、昇格・フォイル演出は引き続き通常どおり再生する。

世界は引いた札の中から1枚を均等に選んで決める（2026-09-17 本人の決定。門と札に関係は持たせるが、SSR やフォイルは優先しない。10連なら 9 割は SSR 以外の世界になり、門で中身が推測しにくい。1回召喚は札の世界がそのまま門になるが、本人は問題なしとした。以前は最高レアのフォイル、無ければ最高レアで、SSR を引くと毎回 天界/魔界になっていた）。乱数は保存済みの抽選結果から決まるので、再表示でも同じ門になる。門の色はフォイルが1枚でもあれば金、なければ銅。結果や所持品はこの演出では変更しない。
