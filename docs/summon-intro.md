...
世界は引いた札の中から1枚をランダムに選んで決める（2026-09-17 本人の指示。以前は最高レアのフォイル、無ければ最高レアで、SSR を引くと毎回 天界/魔界になっていた）。乱数は保存済みの抽選結果から決まるので、再表示でも同じ門になる。門の色はフォイルが1枚でもあれば金、なければ銅。結果や所持品はこの演出では変更しない。
