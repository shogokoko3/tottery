...
> **届いた手の「指し手」は、手の中身ではなく送り主から決めること。**
> 盤を進める手の多くは席番号を名乗りません（`MOVE_PIECE` など11種）。
> 名乗りだけを照らしていると、名乗らない手は素通りし、相手はこちらの手番を
> 勝手に指せます。対局は二人しかいないので、届いた手の指し手は必ず「相手の席」です。
> 受け取った時点で書き込んでしまえば（`src/net/sync.js` の `acceptAct`）、
> 盤（reducer）はそれを照らせます。
>
> **そのうえで、届いた手が合法かどうかも盤で確かめること。** データベースの
> ルールは盤を知らないので、そこでは止められません。`getLegalMoves` を
> 引き直して着地点と取る駒を照らさないと、相手は盤の反対側から王を取れます。

> **保証は `.write` に書くこと。`.validate` は防壁になりません。**
> `.validate` は「書いた場所とその下」しか見ないので、書き込みを1段深くすると
> 祖先になって評価されません。`ranks/<uid>` に形の決まりを `.validate` で書いても、
> `ranks/<uid>/name/x` へ書けば全部外れます。`.write` は祖先として毎回評価される
> ので、そこに置いたものだけが守りになります。ここを取り違えて一度作り直しました。

| 置き場 | 読める人 | 書ける人 |
| --- | --- | --- |
| **`rooms/<合言葉>`** | **席についた二人**＋**名乗った観戦者**（`spectate` が真の部屋のみ）（と運営） | **席についた二人だけ**（手番は追記のみ。観戦者は `spectators/<自分>` の真偽だけ） |
| `lobby` | サインイン済みの人 | 掲示した本人（時間切れの掲示は誰でも片付け可） |
| `ranks/<uid>` | **サインイン済みの人** | 本人と運営 |
| `reports/<id>` | **運営だけ** | 作るのは誰でも／消せるのは運営だけ |
| `players/<uid>` | **本人と運営だけ** | 本人と運営 |
| **`bans/<uid>`** | **本人と運営だけ** | **運営だけ** |

> **2026-09-08 追記(盤面エリア)**: `rooms/<合言葉>/acts/<id>` に4つの欄を足した。
> `hit`(真偽)、`picks`(駒 id の列)、`areas`(真偽)、`loadouts`(両者の装備 `[{rank: skinId}, …]`)。
> `USE_AREA` と `START_SETUP` が運ぶ。**公開はクライアントより先**(古い欄しか知らない
> ルールのままだと、新しいクライアントの手が弾かれて二人の盤がずれる)。公開したら
> `src/game/rule-version.js` の `GAME_RULE_VERSION` を 3 にしてクライアントを出す。
> `tools/check-acts-fit.mjs` が、9×9 の盤面エリアつき対局の手もルールを通ることを見張る。
...
以前は「サインインしていれば誰でも読み書きできる」だった。合言葉は4文字なので、
総当たりで見つかる。見つけた部屋では、

- 手番の列に伏せた王がそのまま入っているので、**相手の王が分かる**
- `{"type":"RESIGN","player":0}` を1件放り込むだけで、**相手を強制的に負けにできる**

`rooms/<合言葉>/seats/{host,guest}` に席を持たせ、座った二人だけが読み書きできるように
した。席は `host` と `guest` の2つしか無いので、数えなくても満室が決まる。座れるのは
`guest` の席に自分の uid を書くときだけ。手番には `by`（指した人の uid）を
持たせ、ルールが `by === auth.uid` を確かめるので、相手の名前は騙れない。

部屋の中身は席についてからでないと読めないので、参加する側は**先に席をとってから**
中を見る。合言葉が違えば席がとれず、そこで「見つかりません」になる。

#### 席に着ける相手を絞った理由

席が空いているあいだは、合言葉さえ分かれば誰でも座れてしまう。座れば部屋が
読め、手番の列に入っている**布陣と伏せた王がそのまま見える**（`SETUP_CONFIRM`
に `placement` と `kingId` が入っている）。しかも「座れるかどうか」がそのまま
「その合言葉の部屋があるかどうか」の判定になるので、総当たりの効率もよい。

- **ランダムマッチの部屋**は、待ち合わせの掲示で名乗った人しか座れないようにした
- **合言葉の部屋**には掲示が無いので、合言葉そのものが鍵になる。4文字
  （約100万通り）では総当たりで抜かれるため、**8文字**（約1.1兆通り）にした

#### 観戦できる部屋を開けた理由（2026-09-24 本人の指示）

フレンドが対戦を観戦できるようにした。ただし**席を増やさない**（第三者が席に着くと
対局を壊せる）。代わりに、対局している本人が設定で観戦をオンにしたときだけ、

- `rooms/<合言葉>/spectate` に `true` を書く（＝この部屋は観戦してよい）。`spectateReveal`
  はフレンド戦=真（両側が見える審判視点）、ランダムマッチ=偽（観戦した席だけ）
- 観戦者は `rooms/<合言葉>/spectators/<自分の uid>` に `true` を書いて名乗る
  （**自分の枝だけ**。`spectate` が真の部屋にしか書けない）
- **読める条件に「席の二人 or 名乗った観戦者」を足した**。手番（`acts`）の**書き込みは
  席の二人だけ**のままなので、観戦者は覗けても指せない

合言葉は台帳（フレンドの在席）を通して**フレンドにだけ**伝わる（`src/server/friends.js`
の `friend_presence`）。掲示のように誰でも読める場所には出さない。手番の列には山札が
そのまま入っているので、観戦者は**両者の手札まで復元できる**（審判視点の土台）。
ランダムマッチで「観戦した席だけ」に見せるのは画面側の約束で、データ上は隠れていない
（真に隠すには `acts` に山札を載せない作りが要る。当事者の再生が壊れるので今はしない）。

> **公開の手順(手動)**: この観戦は **firebase-rules.json を公開して初めて効く**。
> 公開前は観戦者の読みが拒まれ、観戦画面は「見つかりません」で止まる（対局そのものは無事）。

#### 部屋そのものへの書き込みを「作る」と「消す」だけにした理由

ここでも**浅いほうが勝つ**。`rooms/<合言葉>` に中身の書き換えまで許すと、その許可は
下の階層すべてに及び、`acts` に書いた「追記だけ」（`!data.exists()`）を打ち消す。
当事者どうしでも相手の指した手を書き換えられてしまう。

そこで部屋そのものには「作る」と「部屋ごと消す」しか許さず、中身は場所ごとに
許可を出している。参加する側が名乗りを書き足すのは PUT ではなく **PATCH** で、
自分の欄だけを足す（丸ごと置き直すと、それまでの手番の列も消える）。
...
| | やること | 誰が |
| --- | --- | --- |
| 1 | Firebase → Authentication → Sign-in method → **匿名** を有効にする | ご本人 **（済）** |
| 2 | 同じ画面で **メール / パスワード** も有効にする | **ご本人** |
| 3 | Authentication → Users → 「ユーザーを追加」で**運営用のメールとパスワード**を登録 | **ご本人** |
| 4 | `src/net/auth.js` の `API_KEY` にウェブ API キーを入れる | 私 |
| 5 | 管理画面を手元で開き、運営としてサインイン。画面上部に出る **uid** を控える | ご本人 |
| 6 | このファイルの `4jAiZRTJdQTtSJI39JrnpRLtSrB2`（7か所）を uid に置き換える | 私 |
| 7 | キー入りの版を配信し、**みんなが一度開くのを待つ** | — **（済）** |
| 8 | **席と `by` を持つ版**（この変更）を配信する | 私 |
| 9 | Firebase のルール画面に貼って公開する | **ご本人** |

**順番が大事です。** 先にルールを締めると、まだ古い版を開いている人が繋がらなく
なります。とくに 8 は落とせません。新しいルールは部屋に `seats`（席）と、手番に
`by`（指した人）を要求します。古い版はどちらも書かないので、**8 を配信する前に 9 を
公開すると、その時点で開いている人はオンライン対戦ができなくなります。**

貼る前に手元で `npm run check` を通してください。通信はしません。

公開したら `node tools/replay-live.mjs` を動かしてください。本物の対局の手(1000件強)を
使い捨ての口座で本番に本当に書き、Firebase 本体の解釈で全部通るかを見ます。手元の
評価器は Firebase と食い違ったことが二度あります(`numChildren()` はルール言語に無い。
祖先の `.validate` は評価される)。

- `tools/check-rules.mjs`（117件）… ルールの解決を真似て、当事者以外に部屋が
  見えないか・投了を差し込めないか・**書き込みを1段下げても破れないか**・
  PATCH で素通りしないか・使用停止が実際に効くかを確かめます
- `tools/check-accept.mjs`（84件）… 相手から届いた手の守り。指し手が送り主から
  決まること、知らない手を捨てること、そして**本物の対局を作って細工した手を
  実際に流し込み**、盤の反対側の駒が取れないこと・盤が掃討されないこと・
  手番を奪われないこと・落ちないことを確かめます

### 運営の身元にメール＋パスワードを使う理由

匿名の口座は**端末のデータを消すと失われます**。運営権限をそれに紐づけると、
消したとたんに何もできなくなり、ルールを書き直すことになります。
メール＋パスワードなら端末を替えても同じ uid のままです。

管理画面は配信していない（手元でしか開かない）ので、パスワードを打つのは
運営自身の端末だけです。
...
**自動で消えるのは、ルールを公開する前にアプリを開いた人の分だけです。** 公開後は
`players/p…` も `ranks/p…` も本人の uid と一致しないので、その DELETE は必ず弾かれます。
しばらくぶりに開いた人は、古い行が残ったまま新しい行が増えます（ランキングに
同じ人が二重に出ます）。残った `p…` の行は、**管理画面の「消す」で運営が消して
ください**（`players/$uid` と `ranks/$uid` には運営の書き込みを許してあります）。

なお、はじめて遊ぶ人には `p…` の鍵をそもそも作りません。名前を決めるより先に
uid を採るようにしてあります。
...
## まだ塞げていないこと（承知のうえで残している）

App Store へ出す前に、少なくとも「知らないうちに壊される」ことは無い状態にした。
ただし次は、いまの作り（両方の端末が同じ盤を持ち合う。審判役のサーバーが無い）
では原理的に塞げない。**対局相手にだけできること**なので、見知らぬ第三者からは
守られている。

| 何が | なぜ塞げないか |
| --- | --- |
| 対局相手には、こちらの伏せた王と布陣が見えている | 盤を進めるには相手の布陣が要る。両方の端末が同じ盤を持つ作りなので、相手の端末には最初から全部ある |
| 山札・サイコロの目・入れ替えの並びを、振った側が決めて送る | 乱数を決める第三者がいない。両方の端末で同じ盤にするには、どちらかが決めて送るしかない |
| 考えた時間が自己申告 | 受け取る側で測り直していない |
| 1つの手をどれだけ大きくできるか | 欄は全部名指しで、それ以外は `$other: false` で閉じている。配列（山札・布陣・取り・引き・戻し）は**添字の形**（`0`〜`63` など）でしか置けないので枚数も抑わる。ただし手番の**件数**は縛れていない（ルール言語に数える関数が無い）。再戦のたびに列を片付けるので、1局ぶんしか溜まらない | 手番の列が際限なく伸びるのは相手が自分で積む場合だけで、自分の部屋を自分で重くする以上の害は無い |
| ルールで書けることの範囲 | ルール言語には `numChildren()` が**無い**（クライアントの DataSnapshot にはある）。一度これで公開が弾かれた。`tools/check-rules.mjs` が使える関数の名簿を持ち、名簿に無い関数を書くと落ちる | 評価器は JavaScript なので、書けてしまっても気づけない。だから名簿で照らす |
| 使用停止は、端末のデータを消して始め直せば外れる | 匿名の口座はいくらでも作れる。端末を替えても続く本人確認（Sign in with Apple / Game Center）が要る |
| ランキングと待ち合わせを突き合わせると、いま誰が待っているかが分かる | ランキングの鍵が uid で、待ち合わせにも uid が載る。**プライバシーポリシーに明記すること**（App Store 5.1.2） |

いずれも「審判役のサーバーを置く」か「本人確認を入れる」かで直る。
順位に賞品を紐づける、賭けの要素を入れる、といったことをする前には必要になる。
