...
`rooms/<合言葉>/members/<uid>` に席を持たせ、座った二人だけが読み書きできるように
した。席は2つまでで、座れるのは自分の席だけ。手番には `by`（指した人の uid）を
...
なります。とくに 8 は落とせません。新しいルールは部屋に `members`（席）と、手番に
...
| 1つの手をどれだけ大きくできるか | 手の中身は入れ替えや布陣のように入れ子を持つので、ルールで形を決められない。欄の数(24)と件数(1000)までが限界 |
| 1つの手の中身は、欄の数と主な欄の枚数までしか縛れない（形が決まらない欄が残る） | 入れ替えや布陣は入れ子を持つので、知らない名前の子を一律に塞げない |
